namespace Tren.Views {
	partial class CentralDeTratamento1View {
		/// <summary>
		/// Required designer variable.
		/// </summary>
		private System.ComponentModel.IContainer components = null;

		/// <summary>
		/// Clean up any resources being used.
		/// </summary>
		/// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
		protected override void Dispose(bool disposing) {
			if (disposing && (components != null)) {
				components.Dispose();
			}
			base.Dispose(disposing);
		}

		#region Windows Form Designer generated code

		/// <summary>
		/// Required method for Designer support - do not modify
		/// the contents of this method with the code editor.
		/// </summary>
		private void InitializeComponent() {
			this.lb_vazaoMax = new MetroFramework.Controls.MetroLabel();
			this.txb_vazaoMax = new MetroFramework.Controls.MetroTextBox();
			this.lb_vazaoMed = new MetroFramework.Controls.MetroLabel();
			this.lb_vazaoMin = new MetroFramework.Controls.MetroLabel();
			this.lb_vazaoMaxFut = new MetroFramework.Controls.MetroLabel();
			this.lb_vazaoMedFut = new MetroFramework.Controls.MetroLabel();
			this.lb_vazaoMinFut = new MetroFramework.Controls.MetroLabel();
			this.lb_populacao = new MetroFramework.Controls.MetroLabel();
			this.lb_populacaoFut = new MetroFramework.Controls.MetroLabel();
			this.txb_vazaoMed = new MetroFramework.Controls.MetroTextBox();
			this.txb_vazaoMin = new MetroFramework.Controls.MetroTextBox();
			this.txb_vazaoMaxFut = new MetroFramework.Controls.MetroTextBox();
			this.txb_vazaoMedFut = new MetroFramework.Controls.MetroTextBox();
			this.txb_vazaoMinFut = new MetroFramework.Controls.MetroTextBox();
			this.txb_populacao = new MetroFramework.Controls.MetroTextBox();
			this.txb_populacaoFut = new MetroFramework.Controls.MetroTextBox();
			this.lb_centralTitulo = new MetroFramework.Controls.MetroLabel();
			this.bt_central1_avancar = new MetroFramework.Controls.MetroButton();
			this.bt_central2_voltar = new MetroFramework.Controls.MetroButton();
			this.metroLabel1 = new MetroFramework.Controls.MetroLabel();
			this.metroLabel2 = new MetroFramework.Controls.MetroLabel();
			this.metroLabel3 = new MetroFramework.Controls.MetroLabel();
			this.metroLabel4 = new MetroFramework.Controls.MetroLabel();
			this.metroLabel5 = new MetroFramework.Controls.MetroLabel();
			this.metroLabel6 = new MetroFramework.Controls.MetroLabel();
			this.metroLabel7 = new MetroFramework.Controls.MetroLabel();
			this.metroLabel8 = new MetroFramework.Controls.MetroLabel();
			this.SuspendLayout();
			// 
			// lb_vazaoMax
			// 
			this.lb_vazaoMax.AutoSize = true;
			this.lb_vazaoMax.Location = new System.Drawing.Point(31, 107);
			this.lb_vazaoMax.Name = "lb_vazaoMax";
			this.lb_vazaoMax.Size = new System.Drawing.Size(94, 19);
			this.lb_vazaoMax.TabIndex = 0;
			this.lb_vazaoMax.Text = "Vazão Máxima";
			// 
			// txb_vazaoMax
			// 
			this.txb_vazaoMax.Location = new System.Drawing.Point(188, 103);
			this.txb_vazaoMax.Name = "txb_vazaoMax";
			this.txb_vazaoMax.Size = new System.Drawing.Size(137, 23);
			this.txb_vazaoMax.TabIndex = 1;
			// 
			// lb_vazaoMed
			// 
			this.lb_vazaoMed.AutoSize = true;
			this.lb_vazaoMed.Location = new System.Drawing.Point(31, 169);
			this.lb_vazaoMed.Name = "lb_vazaoMed";
			this.lb_vazaoMed.Size = new System.Drawing.Size(84, 19);
			this.lb_vazaoMed.TabIndex = 2;
			this.lb_vazaoMed.Text = "Vazão Média";
			// 
			// lb_vazaoMin
			// 
			this.lb_vazaoMin.AutoSize = true;
			this.lb_vazaoMin.Location = new System.Drawing.Point(31, 234);
			this.lb_vazaoMin.Name = "lb_vazaoMin";
			this.lb_vazaoMin.Size = new System.Drawing.Size(91, 19);
			this.lb_vazaoMin.TabIndex = 3;
			this.lb_vazaoMin.Text = "Vazão Mínima";
			// 
			// lb_vazaoMaxFut
			// 
			this.lb_vazaoMaxFut.AutoSize = true;
			this.lb_vazaoMaxFut.Location = new System.Drawing.Point(31, 299);
			this.lb_vazaoMaxFut.Name = "lb_vazaoMaxFut";
			this.lb_vazaoMaxFut.Size = new System.Drawing.Size(135, 19);
			this.lb_vazaoMaxFut.TabIndex = 4;
			this.lb_vazaoMaxFut.Text = "Vazão Máxima Futura";
			// 
			// lb_vazaoMedFut
			// 
			this.lb_vazaoMedFut.AutoSize = true;
			this.lb_vazaoMedFut.Location = new System.Drawing.Point(413, 107);
			this.lb_vazaoMedFut.Name = "lb_vazaoMedFut";
			this.lb_vazaoMedFut.Size = new System.Drawing.Size(125, 19);
			this.lb_vazaoMedFut.TabIndex = 5;
			this.lb_vazaoMedFut.Text = "Vazão Média Futura";
			// 
			// lb_vazaoMinFut
			// 
			this.lb_vazaoMinFut.AutoSize = true;
			this.lb_vazaoMinFut.Location = new System.Drawing.Point(413, 168);
			this.lb_vazaoMinFut.Name = "lb_vazaoMinFut";
			this.lb_vazaoMinFut.Size = new System.Drawing.Size(132, 19);
			this.lb_vazaoMinFut.TabIndex = 6;
			this.lb_vazaoMinFut.Text = "Vazão Mínima Futura";
			// 
			// lb_populacao
			// 
			this.lb_populacao.AutoSize = true;
			this.lb_populacao.Location = new System.Drawing.Point(413, 234);
			this.lb_populacao.Name = "lb_populacao";
			this.lb_populacao.Size = new System.Drawing.Size(70, 19);
			this.lb_populacao.TabIndex = 7;
			this.lb_populacao.Text = "População";
			// 
			// lb_populacaoFut
			// 
			this.lb_populacaoFut.AutoSize = true;
			this.lb_populacaoFut.Location = new System.Drawing.Point(413, 298);
			this.lb_populacaoFut.Name = "lb_populacaoFut";
			this.lb_populacaoFut.Size = new System.Drawing.Size(111, 19);
			this.lb_populacaoFut.TabIndex = 8;
			this.lb_populacaoFut.Text = "População Futura";
			// 
			// txb_vazaoMed
			// 
			this.txb_vazaoMed.Location = new System.Drawing.Point(188, 165);
			this.txb_vazaoMed.Name = "txb_vazaoMed";
			this.txb_vazaoMed.Size = new System.Drawing.Size(137, 23);
			this.txb_vazaoMed.TabIndex = 9;
			// 
			// txb_vazaoMin
			// 
			this.txb_vazaoMin.Location = new System.Drawing.Point(188, 230);
			this.txb_vazaoMin.Name = "txb_vazaoMin";
			this.txb_vazaoMin.Size = new System.Drawing.Size(137, 23);
			this.txb_vazaoMin.TabIndex = 10;
			// 
			// txb_vazaoMaxFut
			// 
			this.txb_vazaoMaxFut.Location = new System.Drawing.Point(188, 295);
			this.txb_vazaoMaxFut.Name = "txb_vazaoMaxFut";
			this.txb_vazaoMaxFut.Size = new System.Drawing.Size(137, 23);
			this.txb_vazaoMaxFut.TabIndex = 11;
			// 
			// txb_vazaoMedFut
			// 
			this.txb_vazaoMedFut.Location = new System.Drawing.Point(570, 103);
			this.txb_vazaoMedFut.Name = "txb_vazaoMedFut";
			this.txb_vazaoMedFut.Size = new System.Drawing.Size(137, 23);
			this.txb_vazaoMedFut.TabIndex = 12;
			// 
			// txb_vazaoMinFut
			// 
			this.txb_vazaoMinFut.Location = new System.Drawing.Point(570, 164);
			this.txb_vazaoMinFut.Name = "txb_vazaoMinFut";
			this.txb_vazaoMinFut.Size = new System.Drawing.Size(137, 23);
			this.txb_vazaoMinFut.TabIndex = 13;
			// 
			// txb_populacao
			// 
			this.txb_populacao.Location = new System.Drawing.Point(570, 230);
			this.txb_populacao.Name = "txb_populacao";
			this.txb_populacao.Size = new System.Drawing.Size(137, 23);
			this.txb_populacao.TabIndex = 14;
			// 
			// txb_populacaoFut
			// 
			this.txb_populacaoFut.Location = new System.Drawing.Point(570, 298);
			this.txb_populacaoFut.Name = "txb_populacaoFut";
			this.txb_populacaoFut.Size = new System.Drawing.Size(137, 23);
			this.txb_populacaoFut.TabIndex = 15;
			// 
			// lb_centralTitulo
			// 
			this.lb_centralTitulo.AutoSize = true;
			this.lb_centralTitulo.FontSize = MetroFramework.MetroLabelSize.Tall;
			this.lb_centralTitulo.FontWeight = MetroFramework.MetroLabelWeight.Regular;
			this.lb_centralTitulo.Location = new System.Drawing.Point(322, 9);
			this.lb_centralTitulo.Name = "lb_centralTitulo";
			this.lb_centralTitulo.Size = new System.Drawing.Size(161, 25);
			this.lb_centralTitulo.TabIndex = 16;
			this.lb_centralTitulo.Text = "Entrada de Dados";
			// 
			// bt_central1_avancar
			// 
			this.bt_central1_avancar.Location = new System.Drawing.Point(453, 393);
			this.bt_central1_avancar.Name = "bt_central1_avancar";
			this.bt_central1_avancar.Size = new System.Drawing.Size(237, 45);
			this.bt_central1_avancar.TabIndex = 17;
			this.bt_central1_avancar.Text = "Avançar";
			this.bt_central1_avancar.Click += new System.EventHandler(this.bt_central1_avancar_Click);
			// 
			// bt_central2_voltar
			// 
			this.bt_central2_voltar.Location = new System.Drawing.Point(110, 393);
			this.bt_central2_voltar.Name = "bt_central2_voltar";
			this.bt_central2_voltar.Size = new System.Drawing.Size(237, 45);
			this.bt_central2_voltar.TabIndex = 18;
			this.bt_central2_voltar.Text = "Voltar";
			this.bt_central2_voltar.Click += new System.EventHandler(this.bt_central2_voltar_Click);
			// 
			// metroLabel1
			// 
			this.metroLabel1.AutoSize = true;
			this.metroLabel1.Location = new System.Drawing.Point(713, 298);
			this.metroLabel1.Name = "metroLabel1";
			this.metroLabel1.Size = new System.Drawing.Size(54, 19);
			this.metroLabel1.TabIndex = 26;
			this.metroLabel1.Text = "pessoas";
			// 
			// metroLabel2
			// 
			this.metroLabel2.AutoSize = true;
			this.metroLabel2.Location = new System.Drawing.Point(713, 234);
			this.metroLabel2.Name = "metroLabel2";
			this.metroLabel2.Size = new System.Drawing.Size(54, 19);
			this.metroLabel2.TabIndex = 25;
			this.metroLabel2.Text = "pessoas";
			// 
			// metroLabel3
			// 
			this.metroLabel3.AutoSize = true;
			this.metroLabel3.Location = new System.Drawing.Point(713, 168);
			this.metroLabel3.Name = "metroLabel3";
			this.metroLabel3.Size = new System.Drawing.Size(25, 19);
			this.metroLabel3.TabIndex = 24;
			this.metroLabel3.Text = "L/s";
			// 
			// metroLabel4
			// 
			this.metroLabel4.AutoSize = true;
			this.metroLabel4.Location = new System.Drawing.Point(713, 107);
			this.metroLabel4.Name = "metroLabel4";
			this.metroLabel4.Size = new System.Drawing.Size(25, 19);
			this.metroLabel4.TabIndex = 23;
			this.metroLabel4.Text = "L/s";
			// 
			// metroLabel5
			// 
			this.metroLabel5.AutoSize = true;
			this.metroLabel5.Location = new System.Drawing.Point(331, 299);
			this.metroLabel5.Name = "metroLabel5";
			this.metroLabel5.Size = new System.Drawing.Size(25, 19);
			this.metroLabel5.TabIndex = 22;
			this.metroLabel5.Text = "L/s";
			// 
			// metroLabel6
			// 
			this.metroLabel6.AutoSize = true;
			this.metroLabel6.Location = new System.Drawing.Point(331, 234);
			this.metroLabel6.Name = "metroLabel6";
			this.metroLabel6.Size = new System.Drawing.Size(25, 19);
			this.metroLabel6.TabIndex = 21;
			this.metroLabel6.Text = "L/s";
			// 
			// metroLabel7
			// 
			this.metroLabel7.AutoSize = true;
			this.metroLabel7.Location = new System.Drawing.Point(331, 169);
			this.metroLabel7.Name = "metroLabel7";
			this.metroLabel7.Size = new System.Drawing.Size(25, 19);
			this.metroLabel7.TabIndex = 20;
			this.metroLabel7.Text = "L/s";
			// 
			// metroLabel8
			// 
			this.metroLabel8.AutoSize = true;
			this.metroLabel8.Location = new System.Drawing.Point(331, 107);
			this.metroLabel8.Name = "metroLabel8";
			this.metroLabel8.Size = new System.Drawing.Size(25, 19);
			this.metroLabel8.TabIndex = 19;
			this.metroLabel8.Text = "L/s";
			// 
			// CentralDeTratamento1View
			// 
			this.AutoScaleDimensions = new System.Drawing.SizeF(6F, 13F);
			this.AutoScaleMode = System.Windows.Forms.AutoScaleMode.Font;
			this.BackColor = System.Drawing.SystemColors.ControlLightLight;
			this.ClientSize = new System.Drawing.Size(800, 450);
			this.Controls.Add(this.metroLabel1);
			this.Controls.Add(this.metroLabel2);
			this.Controls.Add(this.metroLabel3);
			this.Controls.Add(this.metroLabel4);
			this.Controls.Add(this.metroLabel5);
			this.Controls.Add(this.metroLabel6);
			this.Controls.Add(this.metroLabel7);
			this.Controls.Add(this.metroLabel8);
			this.Controls.Add(this.bt_central2_voltar);
			this.Controls.Add(this.bt_central1_avancar);
			this.Controls.Add(this.lb_centralTitulo);
			this.Controls.Add(this.txb_populacaoFut);
			this.Controls.Add(this.txb_populacao);
			this.Controls.Add(this.txb_vazaoMinFut);
			this.Controls.Add(this.txb_vazaoMedFut);
			this.Controls.Add(this.txb_vazaoMaxFut);
			this.Controls.Add(this.txb_vazaoMin);
			this.Controls.Add(this.txb_vazaoMed);
			this.Controls.Add(this.lb_populacaoFut);
			this.Controls.Add(this.lb_populacao);
			this.Controls.Add(this.lb_vazaoMinFut);
			this.Controls.Add(this.lb_vazaoMedFut);
			this.Controls.Add(this.lb_vazaoMaxFut);
			this.Controls.Add(this.lb_vazaoMin);
			this.Controls.Add(this.lb_vazaoMed);
			this.Controls.Add(this.txb_vazaoMax);
			this.Controls.Add(this.lb_vazaoMax);
			this.Name = "CentralDeTratamento1View";
			this.Text = "Central De Tratamento";
			this.ResumeLayout(false);
			this.PerformLayout();

		}

		#endregion

		private MetroFramework.Controls.MetroLabel lb_vazaoMax;
		private MetroFramework.Controls.MetroTextBox txb_vazaoMax;
		private MetroFramework.Controls.MetroLabel lb_vazaoMed;
		private MetroFramework.Controls.MetroLabel lb_vazaoMin;
		private MetroFramework.Controls.MetroLabel lb_vazaoMaxFut;
		private MetroFramework.Controls.MetroLabel lb_vazaoMedFut;
		private MetroFramework.Controls.MetroLabel lb_vazaoMinFut;
		private MetroFramework.Controls.MetroLabel lb_populacao;
		private MetroFramework.Controls.MetroLabel lb_populacaoFut;
		private MetroFramework.Controls.MetroTextBox txb_vazaoMed;
		private MetroFramework.Controls.MetroTextBox txb_vazaoMin;
		private MetroFramework.Controls.MetroTextBox txb_vazaoMaxFut;
		private MetroFramework.Controls.MetroTextBox txb_vazaoMedFut;
		private MetroFramework.Controls.MetroTextBox txb_vazaoMinFut;
		private MetroFramework.Controls.MetroTextBox txb_populacao;
		private MetroFramework.Controls.MetroTextBox txb_populacaoFut;
		private MetroFramework.Controls.MetroLabel lb_centralTitulo;
		private MetroFramework.Controls.MetroButton bt_central1_avancar;
		private MetroFramework.Controls.MetroButton bt_central2_voltar;
		private MetroFramework.Controls.MetroLabel metroLabel1;
		private MetroFramework.Controls.MetroLabel metroLabel2;
		private MetroFramework.Controls.MetroLabel metroLabel3;
		private MetroFramework.Controls.MetroLabel metroLabel4;
		private MetroFramework.Controls.MetroLabel metroLabel5;
		private MetroFramework.Controls.MetroLabel metroLabel6;
		private MetroFramework.Controls.MetroLabel metroLabel7;
		private MetroFramework.Controls.MetroLabel metroLabel8;
	}
}